...
//用GetNextValidIndex找到下一个有效的字母位置，也就是不包含#，并且已经Backspace        
//如果两个都走到了头，说明前面的字符都匹配成功了
//如果其中一个走到了头，或者指向的字符不相等，直接返回 false
//最终返回 true，表示两个字符串相等。
...
//空间复杂度：O(1)
        public bool BackspaceCompare(string s, string t)
        {
            ...
            while (sPtr >= 0 || tPtr >= 0)
            {                
                sPtr = GetNextValidIndex(s, sPtr);                
                tPtr = GetNextValidIndex(t, tPtr);

                // 如果两个都走到了头，说明前面的字符都匹配成功了
                if (sPtr < 0 && tPtr < 0) return true;
