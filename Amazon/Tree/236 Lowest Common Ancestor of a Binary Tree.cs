...
//“两个节点之间有最近公共祖先p，且q是 中的最低节点，T且同时具有p和q作为后代（其中我们允许一个节点是其自身的后代）”。
//思路：DFS，递归终止条件，即如果 root 为空或者等于 p 或 q，则直接返回 root。然后递归地在左子树和右子树中寻找目标节点 p 和 q。
//如果左子树返回的结果不为空且右子树返回的结果也不为空，则说明 p 和 q 分别位于当前节点的左右子树中，那么当前节点就是它们的最低公共祖先。
//如果左子树返回的结果为空，则说明 p 和 q 都在右子树中，那么返回右子树的结果即可。反之亦然。
