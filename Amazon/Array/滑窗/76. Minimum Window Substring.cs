...
//思路：双指针。使用两个指针 left 和 right 分别指向字符串的开头。
//维护一个字典 charCountT 来记录字符串 t 中每个字符的出现次数。
//使用一个计数器 count 来记录当前窗口中包含 t 中字符的数量。
//遍历字符串 s，将 right 右移，更新窗口状态，
//同时更新 count。当 count 等于 t 中字符的种类数时，说明当前窗口包含了 t 中所有字符，
//此时跟minLenth做比较，right - left + 1 < minLen，满足条件，更新
//然后将 left 右移，缩小窗口，同时更新 count。
//时间复杂度:  n 是字符串的长, O(n)
//空间复杂度： O(26) = O(1)
