...
//思路：使用四个变量 top, bottom, left, right 分别表示当前矩阵的上、下、左、右边界。
//创建一个队列用于按照顺序存储需要遍历的元素，初始时将矩阵的第一行放入队列中。
//进入循环，从队列中取出一行元素，依次将其添加到结果数组中，并根据当前的边界更新 top, bottom, left, right。
//将下一行的元素按照相应的方向（从左到右、从上到下、从右到左、从下到上）依次放入队列中。
