...
//Solution是给一个数组w，数组里面的数组表示它的比重，PickIndex是从给入的数组中随机抽取并显示抽取数字的位置；
//所以随机出现i的概率是：w[i] / sum(w)
//思路：相当于给的数组[1，3，2，1]可以转换成前缀之和[0，1，4，6，7]
//可以理解成随机的数字在[0-7]之间如果出现[0,1]是给入数组位置1，[1，4]之间是给入数组位置2；
//这样就用到了前缀数组和二分搜索里的左侧边界
