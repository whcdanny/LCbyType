...
//题意：在一个二维网格上，每个单元格可以是空白（0）、新鲜橘子（1）或腐烂的橘子（2）。
//腐烂的橘子会以每分钟的速度传播，每个新鲜橘子会被腐烂橘子相邻的四个方向的腐烂橘子感染。
//要求计算腐烂橘子将全部橘子变腐烂的最短时间。
//思路：BFS（广度优先搜索）将所有腐烂的橘子加入队列，并用一个二维数组表示橘子状态。
//然后，从队列中取出腐烂的橘子，依次感染新鲜橘子，并将被感染的新鲜橘子加入队列。
//重复这个过程，直到队列为空。
//最后，检查是否还有新鲜橘子，如果有，说明有些橘子无法被感染，返回-1；否则，返回感染所有橘子所需的分钟数。
