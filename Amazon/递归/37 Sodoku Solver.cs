...
//题意：填充数独的问题
//思路：深度优先搜索（DFS）: 遍历数独格子，当遇到空格时，尝试填充数字 1 到 9，并进行递归搜索。
//在递归搜索中，我们使用上述编写的函数来判断当前数字是否合法，如果合法就继续填充下一个空格。
//如果递归搜索的过程中发现填充的数字无法满足数独规则，就需要回溯，将当前位置重新设为空格。
//同一行是否已经存在数字, 同一列是否已经存在数字,检查同一个 3x3 子格子内是否已经存在数字
//board[row][i] == c; board[i][col] == c; board[3 * (row / 3) + i / 3][3 * (col / 3) + i % 3] == c;
