...
//题意：给定一个由 '1'（陆地）和 '0'（水）组成的二维网格地图，计算岛屿的数量。
//岛屿被水包围，并且通过水平方向或垂直方向相邻时被认为是同一岛屿。可以假设网格的四个边均被水包围。
//思路：深度优先搜索（DFS）来解决。
//具体思路如下：遍历整个二维网格，对于每个 '1'（陆地），进行深度优先搜索，将与之相连的所有 '1' 标记为已访问过。
//在深度优先搜索中，递归地将当前位置的上、下、左、右四个相邻位置标记为已访问。统计深度优先搜索的次数，即为岛屿的数量。
