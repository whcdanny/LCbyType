...
//思路：深度优先搜索（DFS）序列化： 首先遍历矩阵的四条边，将与边界相连的 'O' 及其相邻的 'O' 标记为特殊字符（如 '#'）,然后遍历整个矩阵，将所有剩余的 'O' 修改为 'X'，将特殊字符 '#' 恢复为 'O'
//注：因为与边界相邻的0就不会被给位X，所以先历遍四边，然后跟临边0连接的都改成X。
//时间复杂度: O(m * n)
//空间复杂度：O(1)
        public void Solve(char[][] board)
        {
            ...
            for (int j = 0; j < n; j++)
            {
                if (board[0][j] == 'O') DFS_Mark(board, 0, j);
                if (board[m - 1][j] == 'O') DFS_Mark(board, m - 1, j);
            }
        ...
        private void DFS_Mark(char[][] board, int i, int j)
        {
            int m = board.Length;
            int n = board[0].Length;
