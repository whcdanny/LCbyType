...
//最优化空间  
		public int MaxProfit4(int[] prices, int fee)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                dp_i_1 = Math.Max(dp_i_1, temp - prices[i] -fee);//今天选择 rest,今天选择 buy => 我现在手里有股票
            }
            return dp_i_0;           
        }
//动态规划：没有上面空间优秀但是逻辑一样			
        ...
        {            
            ...
            for (int i = 0; i < n; i++)
            {
                if (i - 1 == -1)
                {
                    // base case
                    dp[i, 0] = 0;
                    dp[i, 1] = -prices[i] - fee;//注意只要卖股票就要交手续费；
                    continue;
                }
