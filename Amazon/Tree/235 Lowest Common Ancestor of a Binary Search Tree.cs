...
//思路：深度优先搜索（DFS） BST 的特性，如果 p 和 q 的值都小于当前节点的值，说明 p 和 q 都在当前节点的左子树中，因此继续在左子树中寻找最低公共祖先。
//如果 p 和 q 的值都大于当前节点的值，说明 p 和 q 都在当前节点的右子树中，因此继续在右子树中寻找最低公共祖先。
//如果 p 和 q 的值分别小于和大于当前节点的值，说明当前节点就是最低公共祖先。
