...
//股票问题，给一个股票在一段时间的数值，然后问收入最大；只能交易一次
...
//最优化空间       
        ...
        {            
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                dp_i_1 = Math.Max(dp_i_1, -prices[i]);//今天选择 rest,今天选择 buy => 我现在手里有股票
            }
            return dp_i_0;            
        }
//动态规划：没有上面空间优秀但是逻辑一样		
        ...
        {            
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                dp[i, 0] = Math.Max(dp[i - 1, 0], dp[i - 1, 1] + prices[i]);//今天选择 rest,今天选择 sell => 我把股票卖了手里没有股票             
                dp[i, 1] = Math.Max(dp[i - 1, 1], -prices[i]);//今天选择 rest,今天选择 buy => 我现在手里有股票
            }
            return dp[n - 1, 0];
        }
//穷举：每一位做差值找最大值 不建议用		
