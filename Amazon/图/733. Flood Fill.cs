...
//题意：给定一个表示图像的整数矩阵 image，以及一个起始点 (sr, sc) 和新的颜色值 newColor，
//你需要将起始点所在区域的所有相连节点的颜色都修改为新的颜色值 newColor。然后返回经过处理后的图像。
//思路：（DFS）问题。我们需要从初始坐标 (sr, sc) 出发，对其相邻且颜色相同的像素点进行递归的颜色填充操作。
//记录初始坐标(sr, sc) 处的颜色 originalColor。从初始坐标(sr, sc) 开始，将其颜色修改为新颜色 newColor。
