//Leetcode 2972. Count the Number of Incremovable Subarrays II hard
...
//子数组是数组中连续的非空元素序列。
//思路：双指针左右针，在两端查找严格递增/严格升序子数组”您现在不是计算要删除的子数组的数量以使剩余数组严格增加，而是查看通过从给定数组的两个严格增加的分区中选择前缀和后缀可以形成的有效数组的数量。
//有效的子数组将仅由严格增加的部分组成。
